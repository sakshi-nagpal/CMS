  ...
  angular.module('oc.lazyLoad.uiRouterDecorator', ['ui.router']).config(function($stateProvider) {
    ...
    function lazyBundle(module, files, templateUrl) {
      ...
      return {
        ...
        resolve: ['$templateCache', '$ocLazyLoad', '$q', '$browser', function lazyResolve($templateCache, $ocLazyLoad, $q, $browser) {
     ...
     *  - resolve: (array) minification-safe injection function array
     */
    function lazyBundleDependencies(module, files) {
      var lazyDeferred;
      return {
        resolve: ['$ocLazyLoad', '$q', '$browser', function lazyResolve($ocLazyLoad, $q, $browser) {
          $browser.$$incOutstandingRequestCount();
          lazyDeferred = $q.defer();
          return $ocLazyLoad.load({
            name: module,
      ...
      angular.forEach(views, function (config, name) {
        if (config.lazyModule && config.lazyFiles && config.lazyTemplateUrl) {
          ...
          if(config.dependencies){
            var dependencies = angular.isArray(config.dependencies)? config.dependencies: [config.dependencies];
            dependencies.forEach(function(obj) {
              config.resolve[obj.module] = lazyBundleDependencies(obj.module, obj.files).resolve;
            });
          }
