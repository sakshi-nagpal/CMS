...
( function() {
	...
	function attach( editor ) {
		var config = editor.config,
			...
			layout = ( function() {
				...
				var mode, editable,
                    ...
                    topDockedOffsetY = config.floatSpaceTopDockedOffsetY || 0,
				...
				function changeMode( newMode ) {
					...
					switch ( newMode ) {
						case 'top':
							updatePos( 'absolute', 'top', editorPos.y - spaceHeight - topDockedOffsetY );
							break;
						case 'pin':
				...
				return function( evt ) {
					...
					// |   |                           |   |
					//
					if ( spaceHeight + topDockedOffsetY <= editorRect.top )
						changeMode( 'top' );
		...
		if ( topHtml ) {
			//BALOO - Revised by KESHAV GARG
			//added editor-toolbar class to the top most div
			var floatSpaceTpl = new CKEDITOR.template(
				'<div' +
					' id="cke_{name}"' +
					' class="cke {id} editor-toolbar cke_reset_all cke_chrome cke_editor_{name} cke_float cke_{langDir} ' + CKEDITOR.env.cssClass + '"' +
					' dir="{langDir}"' +
                ...
                floatSpace, toolbarParent;

            if(config.floatingToolbarParentClass) {
                var editorParent = editor.element.getParent();
                toolbarParent = editorParent.findOne('.' + config.floatingToolbarParentClass);
            } else {
                toolbarParent = CKEDITOR.document.getBody();
            }

            floatSpace = toolbarParent.append( CKEDITOR.dom.element.createFromHtml( floatSpaceTpl.output( {
                    content: topHtml,
                    id: editor.id,
            ...
            //BALOO - revised by Mohit Kansal
			//commented the event binding because we don't need to reposition the toolbar to hide on adjust with scroll
